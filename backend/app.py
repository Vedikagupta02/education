...
def career_advice():
    try:
        ...
        try:
            ...
            return jsonify({"success": False, "error": "Invalid API key. Please check your OpenAI API key configuration."}), 401
        except openai.error.APIError as e:
            return jsonify({"success": False, "error": f"OpenAI API error: {str(e)}"}), 500
        except Exception as e:
            logger.error(f"Unexpected error: {str(e)}")
            return jsonify({"success": False, "error": "An unexpected error occurred while processing your request."}), 500
